    ...
    <div className="shadow-md w-full fixed top-0 left-0">
      <div className="md:px-10 py-4 px-7 md:flex justify-between items-center bg-[#1d1d1d]">
        <div className="flex text-2x1 cursor-pointer items-center gap-2">
          <Link to="/" state={{username}}><span className="font-bold text-white">FELLISSE</span></Link>
        </div>
        {/* Menu Icon*/}
        <div
        ...
        >
          {Links.map((link, index) => (
            <li key={index + link.name} className="font-semibold my-7 md:my-0 md:ml-8">
              <Link to='/aboutUs' state={{username}}>{link.name}</Link>
            </li>
          ))}
          {username ? (
            <span className="text-white ml-8">{username}</span>
            ) : (
            <Link to="/Login" className="btn bg-red-800 text-white py-1 px-3 md:ml-8 rounded md:static">
              Log In/Register
            </Link>
            )}
        </ul>
